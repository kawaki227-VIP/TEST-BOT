...
if (!isAdmins) return reply('You Are Not A Admin')	
  if (args[1] == 'second') {
                ...
                setTimeout(() => {
                    ...
                    const open = `*OPEN TIME* THE GROUP WAS OPENED BY TEST-BOT MD TO APPROVED ADMIN\n NOW MEMBERS CAN SEND MESSAGES 🔓`
...
if (!isAdmins) return reply('You Are Not A Admin')	
                ...
                setTimeout(() => {
                    ...
                    const close = `*CLOSE TIME* GROUP CLOSED BY TEST-BOT MD AT APPROVED ADMIN\nNOW ONLY ADMIN CAN SEND MESSAGES 🔐`
...
if (!isAdmins) return reply('You Are Not A Admin')
const G_MUTE = `*GROUP IS CLOSED BY TEST-BOT-MD BOT OWNER*`;
...
if (!isBotAdmins) return reply('You Not A Bot Owner')
if (!isAdmins) return reply('You Are Not A Admin')
const G_UNMUTE = `*GROUP IS OPEN BY TEST-BOT-MD BOT OWNER*`;
...
await conn.sendMessage(from, { react: { text: `✅`, key: mek.key }})
reply('*GROUP ADMIN DEMOTE BY TEST-BOT-MD BOT OWNER*') 
} catch (e) {
reply('*ERROR*')
console.log(e)
}
