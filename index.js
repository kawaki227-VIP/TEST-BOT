...
if (!fs.existsSync(CREDS)) {
  ...
  if (!session.includes("TEST-BOT~")) {
    console.log("❌ Invalid TEST-BOT session format");
    process.exit(1);
  }
  ...
  console.log("♻️ TEST-BOT session restored successfully");
}
...
async function getProfilePicture(sock, jid) {
  try {
    ...
    return 'https://files.catbox.moe/fnqg9u.jpg'; // Default image
  }
}
...
async function connectToWA() {
  ...
  console.log(`🤖 TEST-BOT Connecting with prefix: "${prefix}"`);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅ TEST-BOT Bot connected to WhatsApp!");
      ...
      const aliveMsg = `*╭──────────────●●►*\n> *TEST-BOT CONNECTED SUCCESSFULLY*\n\n> *Type ${prefix}menu to view commands*  \n\n*╭⊱✫ TEST-BOT ✫⊱╮*\n*│✫📂 Bot Name: ${botConfig.BOT_NAME}*\n*│✫🛡️ Owner: ${config.OWNER_NAME}*\n*│✫♻️ Prefix: ${prefix}*\n*│✫🌍 Mode: ${config.MODE}*\n*│✫⏰ Uptime: ${runtime(process.uptime())}*\n*╰──────────────●●►*\n\n> Enjoy Using TEST-BOT`;
      
      // Image URL for connection message
      const imageUrl = 'https://files.catbox.moe/fnqg9u.jpg';
      
      try {
        // Send to owner with image
        sock.sendMessage(ownerNumber[0] + '22781289418@s.whatsapp.net', {
          image: { url: imageUrl },
          caption: aliveMsg
        }).catch(() => {
      ...
      for (const participant of participants) {
        ...
        if (action === 'add') {
          ...
          if (settings.goodbye) {
            try {
              // Get user's profile picture
              const ppUrl = await getProfilePicture(sock, participant).catch(() => 'https://files.catbox.moe/fnqg9u.jpg');
